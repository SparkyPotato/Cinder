...
float& Matrix::GetValue(uint8_t row, uint8_t column)
{
	...
	auto rowArray = reinterpret_cast<float*>(&m_Rows[row]);
...
float Matrix::GetValue(uint8_t row, uint8_t column) const
{
	...
	auto rowArray = reinterpret_cast<const float*>(&m_Rows[row]);
	return rowArray[column];
}
...
bool operator==(const Matrix& first, const Matrix& second)
{
	for (int row = 0; row < 4; row++)
	{
		...
		for (int column = 0; column < 4; column++)
		{
			if (!IsNearlyEqual(row1[column], row2[column])) { return false; }
		}
	}
